...
static int abc_pcie_linkdown_handler(void *ctx)
{
	...
	dev_dbg(dev, "Broadcast link error notification\n");
				   ...
				   const struct pci_device_id *ent)
{
	...
	while (bar < 5) {
		...
		if (!base) {
			dev_dbg(dev, "failed to read BAR%d\n", bar);
			if (bar >= 5)
				goto exit_loop;
			WARN_ON(bar == BAR_0);
		}
...
static pci_ers_result_t abc_slot_reset(struct pci_dev *pdev)
{
	...
	if (result)
		dev_dbg(dev, "%s:PCIe error reporting enable failed res=%d\n",
				__func__, result);
	result = pci_save_state(pdev);
	if (result)
		dev_dbg(dev, "%s:PCIe state saving failed result=%d\n",
				__func__, result);
